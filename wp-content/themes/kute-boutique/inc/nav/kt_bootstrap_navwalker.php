...
/**
 * Class Name: kt_bootstrap_navwalker
...
class kt_bootstrap_navwalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
            ...
            if( $this->megamenu_item_icon_type != "none" ){
            	...
            	if( $this->megamenu_item_icon_type == "image" ){
            		if($this->megamenu_img_icon){
		                $imgicon = wp_get_attachment_image( intval( $this->megamenu_img_icon),'full',array('class'=>'image_icon_1') );
		                $imgicon_2 = wp_get_attachment_image( intval( $this->img_icon_hover),'full',array('class'=>'image_icon_2') );
		                if(!$imgicon_2){
		                	$imgicon_2 = $imgicon;
		                }
		                if( $imgicon ){
		                    $item_output .= '<span class="menu-icon">'.$imgicon."</span>";
		                }
            	...
            	if( $this->megamenu_item_icon_type == "fonticon" ){
            		if( $this->megamenu_item_font_icon && $this->megamenu_item_font_icon !=""){
            			$item_output .= '<span class="menu-icon '.$this->megamenu_item_font_icon.'"></span>';
            		}
            	}
            }
            // Image note
            if( $this->img_note){
            	$img_note = wp_get_attachment_image( intval( $this->megamenu_img_icon),'full',array('class'=>'image_notice') );
            	if( $img_note ){
			...
			if( $args->has_children ){
				$item_output .= '<span class="item-menu-arow"></span>';
			}
            ...
            if( 'megamenu' == $item->object && $depth == 0){
            	...
            	$item_output .= '<div style="width:'.$mega_menu_width.'px;" class="sub-menu megamenu">'.do_shortcode( $megamenu_item->post_content ).'</div>';
            }
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			$fb_output .= '>';
			$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">' . __( 'Add a menu', 'kute-boutique' ) . '</a></li>';
			$fb_output .= '</ul>';
